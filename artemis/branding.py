...
SPLASH_ASSET_NAME = "_artemis_splash.png"


def ensure_splash_asset(assets_dir: Path):
    """
    Drops Artemis's own logo into the assets folder under a name that
    can't collide with the developer's own branding (`_artemis_splash.png`,
    not `logo.png`) - the dev-mode splash screen is Artemis's mark, not
    the app's, the same way a game engine's own splash is separate from
    the game's icon. Always the bundled default, never the developer's
    custom logo, even if they've set one - see App(splash=...) in app.py
    for the on/off switch instead.
    """
    assets_dir.mkdir(parents=True, exist_ok=True)
    path = assets_dir / SPLASH_ASSET_NAME
    if not path.exists():
        path.write_bytes(_bundled_bytes("logo.png"))
    return SPLASH_ASSET_NAME
