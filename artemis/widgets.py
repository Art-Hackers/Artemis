...
from .theme import resolve_color, resolve_gradient
...
def Box(content=None, pad=16, radius=12, bg=None, gradient=None, glass=False, shadow=None, on_click=None, **kw):
    """
    A styled ft.Container - the general-purpose "put a rounded box around
    this" helper. A few extras that raw Flet makes you assemble by hand:

    - `gradient="sunset"` - a named preset from `art.GRADIENTS` (also
      accepts a raw list like `["#6366F1", "#EC4899"]`, or a list of
      palette names). Either way Artemis builds the LinearGradient
      (top-left to bottom-right) for you.
    - `glass=True` - a frosted-glass panel (blurred backdrop, translucent
      ...
      here it's one flag.
    - `shadow="sm"/"md"/"lg"` (or `True` for "md") - a soft drop shadow,
      the kind of thing that makes a card look like it's actually sitting
      above the page instead of painted flat onto it.
    """
    if glass:
    ...
    if gradient:
        grad = ft.LinearGradient(
            ...
            colors=resolve_gradient(gradient),
        )

    box_shadow = None
    if shadow:
        preset = shadow if isinstance(shadow, str) else "md"
        blur_radius, spread, dy, opacity = _SHADOW_PRESETS.get(preset, _SHADOW_PRESETS["md"])
        box_shadow = ft.BoxShadow(
            blur_radius=blur_radius,
            spread_radius=spread,
            offset=ft.Offset(x=0, y=dy),
...
_SHADOW_PRESETS = {
    # blur_radius, spread_radius, y_offset, opacity
    "sm": (8, 0, 2, 0.10),
    "md": (18, 0, 6, 0.14),
    "lg": (32, 0, 14, 0.20),
}
...
def Hero(title, subtitle=None, gradient="sunset", icon=None, height=None, radius=20, **kw):
    """
    A big, full-width gradient banner - the difference between a screen
    that looks like a stack of form fields and one that looks like a
    designed product. Good for onboarding, empty states, feature
    announcements, the top of a landing/home screen.

        art.Hero("Welcome back", "Here's what's new today", gradient="aurora")
        art.Hero("Go Pro", "Unlock every feature", gradient="cyberpunk", icon=ft.Icons.BOLT)

    `gradient` takes any name from `art.GRADIENTS` (or a raw color list,
    same as `Box`).
    """
    items = []
    if icon:
        items.append(ft.Icon(icon, color=ft.Colors.WHITE, size=36))
    items.append(Text(title, size=26, bold=True, color="white"))
    if subtitle:
        items.append(Text(subtitle, size=14, color=ft.Colors.with_opacity(0.88, ft.Colors.WHITE)))
...
def StatCard(label, value, icon=None, color=None, trend=None, **kw):
    """
    A single dashboard stat: a big number, a label underneath, and an
    optional icon badge + trend indicator - the card every analytics or
    admin screen is made of, without hand-assembling Row/Column/Icon
    every single time.

        art.StatCard("Revenue", "$12,400", icon=ft.Icons.TRENDING_UP, trend="+12%", color="emerald")
    """
    accent = resolve_color(color) or ft.Colors.PRIMARY
    top_row = []
    if icon:
        top_row.append(
            ft.Container(
    ...
    if trend:
        top_row.append(Text(trend, size=12, bold=True, color=accent))

    parts = []
    if top_row:
        parts.append(Row(top_row, gap=8))
    parts.append(Text(str(value), size=26, bold=True))
    parts.append(Text(label, muted=True, size=13))

    return Card(Column(parts, gap=6), pad=18, **kw)


def Chip(label, color=None, selected=False, leading=None, on_click=None, **kw):
    """
    A small pill-shaped tag - filter chips, category labels, a selected
    state. Wraps ft.Chip.

        art.Chip("New", color="rose")
        art.Chip("Design", selected=True, on_click=lambda e: toggle_filter("design"))
    """
    return ft.Chip(
        label=Text(label),
        leading=leading,
        selected=selected,
        bgcolor=resolve_color(color),
        on_click=_after(on_click) if on_click else None,
        **kw,
    )


def IconCircle(icon, color=None, gradient=None, size=48, icon_size=None, **kw):
    """
    An icon centered in a colored (or gradient) circle - the "feature
    highlight" pattern used in every onboarding screen and settings menu.

        art.IconCircle(ft.Icons.BOLT, color="amber")
        art.IconCircle(ft.Icons.STAR, gradient="sunset", size=64)
    """
    return Box(
        ft.Icon(icon, color=ft.Colors.WHITE, size=icon_size or size * 0.5),
        bg=resolve_color(color) if not gradient else None,
        gradient=gradient,
        radius=size / 2,
        pad=0,
        width=size,
