...
def pick(name):
    # call set_theme with only the palette name so we don't explicitly
    # pass `None` for background/surface/text (that would clear any
    # existing overrides). Leaving those kwargs off keeps their current values.
    return lambda e: app.set_theme(name)
...
def home(page):
    return art.Column(
        [
            art.Title("Custom theme", color="#1B5BDD"),
