...
class App:
    ...
    def __init__(self, title="Artemis App", theme="indigo", font=None,
        ...
        self._file_picker = None  # lazily created on first pick_files()/save_file() call
    ...
    def page(self, route="/", title=None, appbar=True):
        ...
        """
        def register(fn):
            self._routes[route] = {"handler": fn, "title": title, "appbar": appbar, "regex": None}
            return fn
        return register
    ...
    def _ensure_file_picker(self):
        if self._file_picker is None:
            ...
            # the client has to actually learn this control exists before we
            # can call a method on it, or it errors with "Unknown control:
            # FilePicker" - this update() is what makes that handshake happen
            self._page.update()
        return self._file_picker

    def pick_files(self, on_result, allow_multiple=False, allowed_extensions=None, dialog_title=None):
        """
        Returns an on_click-ready handler that opens the OS file picker and
        calls `on_result(files)` once the user's done - `files` is a plain
        list (empty if they cancelled), each with `.name`, `.path`,
        `.size`. In raw Flet this needs a FilePicker control manually added
        to `page.overlay` before you can use it; Artemis creates and
        registers it for you the first time you call this.

            art.Button("Upload", on_click=app.pick_files(handle_files, allowed_extensions=["png", "jpg"]))
        """
        async def handler(e):
    ...
    def save_file(self, on_result, file_name=None, allowed_extensions=None, dialog_title=None):
        """Same idea as pick_files but for the OS "save as" dialog -
        `on_result(path)` gets a string path (or None if cancelled)."""
        async def handler(e):
            picker = self._ensure_file_picker()
    ...
    def pick_folder(self, on_result, dialog_title=None):
        """Same idea again, but for picking a directory - `on_result(path)`
        gets a string path (or None if cancelled)."""
        async def handler(e):
            picker = self._ensure_file_picker()
