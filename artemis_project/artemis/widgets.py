...
def Button(text, on_click=None, variant="filled", icon=None, width=None, expand=False, **kw):
    kind = _BUTTON_KINDS.get(variant.lower(), ft.FilledButton)
    return kind(
        text,
        icon=icon,
        on_click=_after(on_click),
        width=width,
        expand=expand,
        **kw,
    )
...
def Input(label="", value="", bind=None, password=False, multiline=False, on_change=None, width=None, **kw):
    """
...
def toast(message, bg=None, seconds=3):
    """
